...
| **E1** Offline eval smoke guards merge; `model-quality-gate` owns promotion `[agentic]` **(load-bearing)** | PASS | `eval/run_eval.py` deterministically gates English and Japanese precision/recall plus column accuracy and block-entity recall; `make gate` wires it into CI. It now speaks the fleet's `EvalReport`, so the fail-closed `passed` rule applies here too, and the six bars have moved out of module constants into `eval/rubrics/*.yaml` beside the argument for each, with `assert_covers` failing the build in both directions. `agent-eval-kit` is in the DEV extra, not `dependencies`: the zero-mandatory-dependency claim is about the product, and the eval is not the product. `model-quality-gate` promotion is N-A by design for this standalone air-gapped tool, and the repo does not claim a fake remote promotion authority, which is also why it is the one repository absent from that authority's `DECLARED_BY` table. |
| **E2** Safety metric with strictest threshold, no false green `[agentic]` | PASS | The eval reports release-critical block-entity recall separately at the strictest threshold (`>=0.99`) across English and Japanese golden labels. It invokes `DlpOrchestrator.scan_text`, the composed path the product runs, over the golden set's WHOLE label set; the independent golden labels are the oracle. A planted scanner that finds nothing drives the score to zero in `test_eval_gate.py`. The runner previously scored `TextDetectionService.scan` behind an `evaluated_entities` filter built from the regex recognizers, which removed the NER analyzer, the OCR path, the image redactor and the Gemma adjudicator from BOTH the expected and the predicted set: a model path could invent any number of findings without lowering precision. `test_a_non_regex_false_positive_now_lowers_precision` is the proof that it no longer can. |
...
| **E4** Promotion verdict fails closed when nothing was evaluated `[agentic]` **(load-bearing)** | PASS | `eval_text_path` returns recall `0.0` on a golden set with no positive case and precision `0.0` on one with no negative case, so an empty or one-sided corpus scores zero rather than a vacuous 1.0, and `test_eval_gate.py` drives both. Beyond the numbers, an unmeasured detection path is now PRINTED as `UNMEASURED` rather than filtered out of the scoring, which is the same rule applied to coverage: a path with no measurement must not be indistinguishable from a path that passed. |
| **E5** The gate command is this repo's own `[all]` | PASS | `make gate` (`lint test eval portability`) is the recipe and is what `gate.yaml` runs. |
...
recipe. GitHub Actions became the fleet's live CI on 2026-09-02; how far this residual is now
proved is `org-metadata/docs/deployment-status.md`'s to say. A7 rests on the kernel/vertical
...
- **Hosted CI dependency-vulnerability evidence (load-bearing residual: D2).** Lockfiles,
  ...
  evidence gap. GitHub Actions became the fleet's live CI on 2026-09-02; how far this residual is
  now proved is `org-metadata/docs/deployment-status.md`'s to say, not this per-repository copy.
