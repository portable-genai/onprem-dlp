...
those resolve to 12 PASS, 1 PARTIAL (D2, externally blocked), 0 FAIL, 2 N-A-by-design (C1, C2). The gate itself
...
| **D2** Digest-pinned images, SHA-pinned Actions, dependabot, CI audit `[all]` **(load-bearing)** | PARTIAL | The base image is digest-pinned, every GitHub Action is pinned by a full commit SHA, and Dependabot tracks pip/docker. The locked networked recipe installs the toolchain and runs exactly pinned `pip-audit==2.10.1` strictly over both fully pinned runtime and development locks with resolver use disabled. Both audits pass locally. The catalog implements this repo's exact two-job GCP hosted-CI contract with secret-free PR/main triggers, but does not apply it. The residual is externally blocked and entirely non-code: an **operator** must (1) apply the two-job hosted-CI contract to this repo, (2) set branch protection on `main` requiring its PR check, and (3) retain one successful hosted execution of the exact pinned `pip-audit==2.10.1 --strict --no-deps --disable-pip` recipe over both lockfiles. GitHub Actions are disabled fleet-wide by portfolio decision, so no push produces a run; nothing in this repository can close this row. |
