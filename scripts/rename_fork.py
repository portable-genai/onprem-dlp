...
Rewrites the package name, the CLI entry point, the ``ONPREM_DLP_`` env-var prefix, and
...
(``onprem-dlp``), so a single kebab id typically covers all three; ``--dist`` defaults to
``--resource`` rather than a ``doc-``/``rsk-`` prefixed name. Pass ``--dist`` explicitly if
your fork's distribution name differs from its resource stem.
...
(pattern packs, golden set, fixtures, egress policy, hash salt) that this script does NOT
do for you.
"""
...
_OLD_PACKAGE = "onprem_dlp"
_OLD_CLI = "onprem-dlp"
_OLD_ENV_PREFIX = "ONPREM_DLP_"
_OLD_RESOURCE = "onprem-dlp"
_OLD_DIST = "onprem-dlp"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``ONPREM_DLP_`` when it prefixes an UPPER token
        # (ONPREM_DLP_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument("--resource", required=True, help="new resource id stem, e.g. acme-dlp")
    ...
    print(
        "Then finish the human steps in docs/ADOPTING.md (pattern packs, golden set, policy, salt)."
    )
    return 0
